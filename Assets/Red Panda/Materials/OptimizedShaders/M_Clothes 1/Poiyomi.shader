Shader "Hidden/Locked/.poiyomi/Old Versions/8.0/Poiyomi Toon/cb43d14ca3de04342908ed0370d83270"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float MainColorFeatherStep = (0.5 /*_BaseColor_Step*/) - (0.375 /*_BaseShade_Feather*/);
				float firstColorFeatherStep = (0.416 /*_ShadeColor_Step*/) - (0.511 /*_1st2nd_Shades_Feather*/);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				firstShadeMap.rgb *= float4(0.2960923,0.2509172,0.301212,1).rgb; //* lighColor
				secondShadeMap.rgb *= float4(0.1034845,0.07311054,0.09609537,1).rgb; //* LightColor;
				float shadowMask = 1;
				...
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / ((0.416 /*_ShadeColor_Step*/) - firstColorFeatherStep) * (shadowMask));
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_EmissionMapUV*/)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0.5,0.5,0.5,0).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * (5.0 /*_EmissionCenterOutSpeed*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0.5,0.5,0.5,0).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0.5,0.5,0.5,0).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				#endif
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, (0.4 /*_RimWidth*/));
				...
				if ((1.0 /*_RimHueShiftEnabled*/))
				{
					rimColor.rgb = hueShift(rimColor.rgb, _RimHueShift + _Time.x * (0.0 /*_RimHueShiftSpeed*/));
				}
				rimWidth = max(lerp(rimWidth, rimWidth * lerp(0, 1, poiLight.lightMap - _ShadowMixThreshold) * _ShadowMixWidthMod, _ShadowMix), 0);
				float rim = 1 - smoothstep(min((0.15 /*_RimSharpness*/), rimWidth), rimWidth, viewDotNormal);
